...
export function useDataSurat(printPath: string) {
  ...
  const filteredData = useMemo(() => data.filter(reg => {
    const matchDate = filterDate ? format(new Date(reg.tanggalTerima), "yyyy-MM-dd") === filterDate : true
  ...
  const groupedData = useMemo(() => filteredData.reduce((acc: Record<string, RegisterSurat[]>, reg) => {
    const dateKey  = reg.tanggalTerima ? format(new Date(reg.tanggalTerima), "dd MMMM yyyy", { locale: id }).toUpperCase() : "TANPA TANGGAL"
  ...
  const sortedGroupKeys = useMemo(() => Object.keys(groupedData).sort((a, b) => {
