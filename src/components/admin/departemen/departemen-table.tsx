...
export function DepartemenTable({
  ...
  return (
    <div className="hidden overflow-hidden rounded-xl border border-border/40 bg-background md:block">
      <div className="grid grid-cols-[180px_minmax(0,1fr)_150px] border-b border-border/40 bg-muted/40 px-4 py-3 text-[12px] font-medium text-muted-foreground">
        <div>ID</div>
      ...
      {isLoading ? (
        ...
        departments.map((departemen, index) => (
          <div
            key={departemen.id}
            className="grid grid-cols-[180px_minmax(0,1fr)_150px] items-center border-b border-border/40 last:border-b-0"
          >
            <div
              role="button"
              tabIndex={0}
              className="col-span-3 grid cursor-pointer grid-cols-[180px_minmax(0,1fr)_150px] items-center px-4 py-3 outline-none transition-colors focus-visible:bg-muted/40"
