      ...
      <table className="pt">
        ...
        <tbody>
          {groups.map((group: CetakGroup) => {
            const totalDetailRows = group.registers.reduce((sum, reg) => {
              return sum + (reg.detailSurat?.length || 1)
            }, 0)
                  ...
                  return details.map((detail: DetailSurat, idx: number) => {
                    ...
                    return (
                      <tr key={`${reg.id}-${idx}`}>
                        {idx === 0 && (
                          <>
                            {/* ✅ class td-nomor — rata kiri, tidak di-override CSS global */}
                            <td rowSpan={span} className="td-nomor">
                              <b>{reg.nomor}</b>
                            </td>
                        ...
                        {isVeryFirstRow && (
                          <td rowSpan={totalDetailRows} className="td-ttd"></td>
                        )}
                      </tr>
                    )
                  })
