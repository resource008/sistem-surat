...
export function RegisterInfoPanel({ state, actions }: any) {
  const nomorRegister = state.previewNomor ?? "N/A"
  ...
  return (
    <div className="w-full lg:w-4/12 xl:w-4/12 flex flex-col gap-4 lg:h-full lg:pb-6">
      <div className="rounded-2xl border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 overflow-hidden flex flex-col max-h-full">
        <div className="px-6 py-4 bg-slate-50 dark:bg-neutral-900 border-b border-slate-200 dark:border-neutral-800 shrink-0">
          <div className="flex items-start justify-between gap-3">
            ...
            <Badge className="shrink-0 mt-0.5 text-[12px] font-medium px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-neutral-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-neutral-700">
              {deptBadge}
            </Badge>
          </div>
        </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-4 lg:overflow-y-auto [&::-webkit-scrollbar]:hidden">
          <FormField label="Departemen">
            <Select value={state.deptId} onValueChange={actions.setDeptId}>
              ...
              <SelectContent className="dark:bg-neutral-950 border-slate-200 dark:border-neutral-800">
          ...
          <FormField label="Tujuan">
            <div className={cn(
              "w-full px-3.5 h-10 flex items-center rounded-xl border text-[14px] font-medium border-slate-200 dark:border-neutral-800 bg-slate-50 dark:bg-neutral-900 cursor-not-allowed select-none",
              state.selectedDept
          ...
          <FormField label="Tanggal Terima">
            <div className={cn(readonlyClass, "w-full px-3.5 h-10 flex items-center rounded-xl border text-[14px] font-medium border-slate-200 dark:border-neutral-800 bg-slate-50 dark:bg-neutral-900 text-slate-400 dark:text-slate-500 cursor-not-allowed select-none")}>
