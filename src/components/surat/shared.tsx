...
export function DatePicker({
  value, onChange, placeholder = "Pilih tanggal", hasError = false, disabled = false,
}: {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {value ? format(selected!, "dd MMMM yyyy", { locale: localeID }) : placeholder}
        </Button>
      </PopoverTrigger>
...
export function FormField({
  label, optional = false, hint, error, children,
}: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        ...
        {optional && <span className="text-[10px] normal-case font-normal text-slate-400">(opsional)</span>}
        {hint    && <span className="text-[10px] normal-case font-normal text-blue-500">{hint}</span>}
      </div>
      {children}
      {error && (
        <p className="text-[11px] text-red-500 dark:text-red-400 font-medium leading-none">{error}</p>
      )}
    </div>
  )
}