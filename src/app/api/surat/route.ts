...
// ─── Handler: RegisterPI + DetailPI ───────────────────────────────────────────

async function handlePI({
  body, parsedTanggal, asalSurat, tujuan, deptId,
}: {
  body: any
  dept: any
  parsedTanggal: Date
  ...
  // Hitung nomor berikutnya dari RegisterPI
  const lastRegister = await prisma.registerPI.findFirst({
    where: { deptId },
    orderBy: { nomor: "desc" },
    select: { nomor: true },
  })
      ...
      detailPI: {
        create: piList.map((p: any) => {
          ...
          if (isNaN(tglSurat.getTime())) {
            throw new Error(`tanggalSurat tidak valid: ${p.tanggalSurat}`)
          }
          return {
...
// ─── Handler: RegisterSurat + DetailSurat ─────────────────────────────────────
...
async function handleSurat({
  ...
  // Hitung nomor berikutnya dari RegisterSurat
    ...
    data: {
      ...
      detailSurat: {
        create: suratList.map((s: any) => {
          ...
          if (isNaN(tglSurat.getTime())) {
            throw new Error(`tanggalSurat tidak valid: ${s.tanggalSurat}`)
          }
          return {
            perihal      : s.perihal,
