...
export default function TambahSuratPage({ role, basePath }: Props) {
  ...
  useEffect(() => {
    ...
    const deptData = deptList.find(d => d.shortName === form.deptId)
    if (!deptData) {
      setPreviewNomor(null)
      return
    }
          ...
          <div className="p-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Departemen">
                <Select value={form.deptId} onValueChange={v => setForm(prev => ({ ...prev, deptId: v }))}>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button type="button" variant="outline"
                      className={cn(inputClass, "h-10 justify-start text-left shadow-none",
                        !form.tanggalTerima && "text-slate-400 dark:text-slate-500")}>
                      <CalendarIcon className="mr-2 h-4 w-4 text-slate-500" />
                      {form.tanggalTerima
                        ? format(new Date(form.tanggalTerima), "dd MMM yyyy", { locale: localeID })
                        : "Pilih tanggal"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0 border-slate-200 dark:border-slate-800" align="start">
                    <Calendar mode="single"
                      selected={form.tanggalTerima ? new Date(form.tanggalTerima) : undefined}
                      onSelect={d => setForm(prev => ({ ...prev, tanggalTerima: d?.toISOString().slice(0, 10) || "" }))}
                      initialFocus />
                  </PopoverContent>
                </Popover>
              </FormField>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button type="button" variant="outline"
                      ...
                      <CalendarIcon className="mr-2 h-4 w-4 text-slate-500" />
                      {form.tanggalSurat
                        ? format(new Date(form.tanggalSurat), "dd MMM yyyy", { locale: localeID })
                        : "Pilih tanggal"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0 border-slate-200 dark:border-slate-800" align="start">
                    <Calendar mode="single"
                      ...
                      onSelect={d => setForm(prev => ({ ...prev, tanggalSurat: d?.toISOString().slice(0, 10) || "" }))}
                      initialFocus />
                  </PopoverContent>
                </Popover>
              </FormField>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="Tujuan" hint="· otomatis">
