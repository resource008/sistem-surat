...
// ── Context agar users-page bisa baca search ─────────────────

interface SearchCtx { search: string; setSearch: (v: string) => void }
export const SearchContext = createContext<SearchCtx>({ search: "", setSearch: () => {} })
export const useSearch = () => useContext(SearchContext)

// ── Search input di topbar ────────────────────────────────────

function TopbarSearch({ isMobile, onExpand }: { isMobile: boolean; onExpand?: (v: boolean) => void }) {
  ...
  // Mobile — ikon dulu, expand saat diklik
  if (!expanded) {
    return (
      <button
  ...
  // Mobile — expanded: input full width + tombol X
  return (
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Reset searchExpanded saat navigasi
  useEffect(() => {
    setSearchExpanded(false)
  }, [pathname])

  // Reset search saat keluar dari halaman users
  useEffect(() => {
  ...
  useEffect(() => {
    const check = () => {
      ...
      if (!mobile) setHideSidebarToggle(false)
    }
    check()
  ...
  // Hide sidebar toggle listener — hanya berlaku di mobile
  useEffect(() => {
    const h = (e: Event) => {
      ...
      <div className={styles.root}>
        ...
        >
          <div id="topbar" className={styles.topbar}>
            {/* Mobile + users + expanded: sembunyikan hamburger & judul */}
